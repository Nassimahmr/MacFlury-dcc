<?php 	
	...
	if(!(empty($_POST['titre']) || empty($_POST['price']) || empty($_FILES['img']['name']) || empty($_POST['p_key_word']) )){
	//post value
	$id=$_POST['update'];
	$titre=$_POST['titre'];
	$price=$_POST['price'];
	...
	//update pro
		...
		$update_pro="UPDATE livres SET titre='$titre', price='$price', img='$img', p_key_word='$p_key_word' WHERE id='$id'";
		$run_pro=mysql_query($update_pro);
			echo "<script>alert ('mise a jour terminé avec succes');</script>";
		   ...
		   <br>
			...
			<div class="box1">
				...
				<form name="f" action="mise-a-jour-livres.php" method="POST" enctype="multipart/form-data">
					...
					<a href="mise-a-jour-livres.php">
					<button type="submit" name="update" value="<?php echo $_GET['update']; ?>" onclick="verif();" style="height: 40px;border-radius: 0px;border: 0px;font-size: larger;">
						Mise a jour livre
					</button></a>
					</div>
				</form>
